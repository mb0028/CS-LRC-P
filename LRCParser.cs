...
{
    public List<LyricLine> LyricLines { get; private set; } = [];
    ...
    public LRCParser(string lrcFilePath, int maxLinesToRead, bool debug = false)
    {
        ...
        if (string.IsNullOrEmpty(lrcFilePath) || !Path.Exists(lrcFilePath))
...
public readonly struct LyricLine(float time, string lyric)
{
    public readonly float TimeStomp = time;
    public readonly string Lyric = lyric;
...
public static partial class LRCParserHelper
{
    [GeneratedRegex(@"\[(\d+):(\d+\.\d+)\](\P{IsGreek}+)")]
    public static partial Regex LRCLine();

    [GeneratedRegex(@"\[(\d+):(\d+\.\d+)\]")]
    public static partial Regex LRCLineNoWord();
